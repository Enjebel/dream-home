...
import { Plus, Home, Building2, AlertTriangle, Trash2, ExternalLink } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchMyListings = async () => {
    try {
      setLoading(true);
      setError(null);
      // Matches the endpoint in your propertyRoutes.js: router.get('/myproperties', protect, getMyProperties);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this listing? This action cannot be undone.")) {
      try {
        ...
        // Filter out the deleted property from state so the UI updates immediately
  ...
  if (loading) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center">
      ...
      <p className="mt-4 font-bold text-gray-500 uppercase tracking-widest">Loading portfolio...</p>
    </div>
  );
  ...
  if (error) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-6">
      ...
      <h2 className="text-2xl font-black text-gray-800">Dashboard Error</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div>
          <h1 className="text-5xl font-black text-gray-900 tracking-tighter italic uppercase">Agent Dashboard</h1>
          <p className="text-gray-500 mt-2 text-lg font-medium flex items-center gap-2">
            <Building2 size={18} className="text-blue-600" />
            Managing {myProperties.length} Active Listings
          </p>
        </div>
        <button 
      ...
      {myProperties.length === 0 ? (
        <div className="bg-white rounded-[40px] p-20 text-center border-2 border-dashed border-gray-100 shadow-sm">
          ...
          <h3 className="text-2xl font-black text-gray-800 mb-2">NO ACTIVE LISTINGS</h3>
          <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">You haven't added any properties yet. Start building your portfolio today.</p>
          <Link to="/add-property" className="text-blue-600 font-black flex items-center justify-center gap-2 hover:underline uppercase tracking-tight">
            Create your first listing <Plus size={18} />
          </Link>
        </div>
      ) : (
            ...
            <div key={prop._id} className="group bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-300 flex flex-col">
              <div className="h-64 relative overflow-hidden">
                <img 
                  ...
                  alt={prop.title} 
                />
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-xl font-black text-blue-600">
                  ${prop.price?.toLocaleString()}
                </div>
              </div>

              <div className="p-8 flex-1 flex flex-col">
                <h3 className="text-xl font-black text-gray-900 mb-1 truncate uppercase tracking-tighter">{prop.title}</h3>
                <p className="text-gray-400 font-bold text-sm mb-6">{prop.location}</p>
